...
if(isset($_POST)){
	...
	foreach( $all_arr as $filter ){
		$filter_arr_a[$filter] = array();
		$filter_arr_b[$filter] = array();
		$q_count_a = "SELECT $filter AS FILTER, DATE( c_timestamp ) AS DAY, COUNT( * ) AS CNT FROM V$hash_number WHERE ab='a' GROUP BY $filter, DATE( c_timestamp );";
		$q_count_b = "SELECT $filter AS FILTER, DATE( c_timestamp ) AS DAY, COUNT( * ) AS CNT FROM V$hash_number WHERE ab='b' GROUP BY $filter, DATE( c_timestamp );";
		// calc A
		if ( $result = $db_obj->query($q_count_a)){
			if( $result->num_rows > 0 ){
				//$row = $result->fetch_assoc();
				while($row = $result->fetch_assoc()){
					$tagstr = $row['FILTER'];
					if($debug == true){echo "filterarr_a[".$filter."][".$tagstr."]=";}
					if(!isset($filter_arr_a[$filter][$tagstr])){
						$filter_arr_a[$filter][$tagstr] = new line();
						$filter_arr_a[$filter][$tagstr]->setKey($tagstr);
					}
					$filter_arr_a[$filter][$tagstr]->addXY($row['DAY'],$row['CNT']);
				}
				$tempArr = array();
				foreach($filter_arr_a[$filter] as $filterItem){
					$tempArr[] = $filterItem->getJSON();
				}
				$tags_arr_a[$filter] = "{".implode(",", $tempArr)."}";
			}else{
				errlog("DB_ERROR_RETURN","db returned 0 rows for filter set A",$filter);
				//sendback(-1);
			}
		}else{
			errlog("DB_ERROR_COUNT","error counting filter set A",$filter);
			errlog("DB_ERROR_COUNT_SQL",mysql_error(),mysql_errno());
			sendback(-1);
		}
		// calc B
		if ( $result = $db_obj->query($q_count_b)){
			if( $result->num_rows > 0 ){
				//$row = $result->fetch_assoc();
				while($row = $result->fetch_assoc()){
					$tagstr = $row['FILTER'];
					if($debug == true){echo "filterarr_b[".$filter."][".$tagstr."]=";}
					if(!isset($filter_arr_b[$filter][$tagstr])){
						$filter_arr_b[$filter][$tagstr] = new line();
						$filter_arr_b[$filter][$tagstr]->setKey($tagstr);
					}
					$filter_arr_b[$filter][$tagstr]->addXY($row['DAY'],$row['CNT']);
				}
				$tempArr = array();
				foreach($filter_arr_b[$filter] as $filterItem){
					$tempArr[] = $filterItem->getJSON();
				}
				$tags_arr_b[$filter] = "{".implode(",", $tempArr)."}";
			}else{
				errlog("DB_ERROR_RETURN","db returned 0 rows for filter set B",$filter);
				//sendback(-1);
			}
		}else{
			errlog("DB_ERROR_COUNT","error counting filter set B",$filter);
			errlog("DB_ERROR_COUNT_SQL",mysql_error(),mysql_errno());
			sendback(-1);
		}
	}
	...
	$prejoin = array();
	foreach($tags_arr_a as $tag => $str){
		$prejoin[] = "\"$tag\":$str";
	}
	$return_arr["tags_a"]="{".join(",",$prejoin)."}";

	$prejoin = array();
	foreach($tags_arr_b as $tag => $str){
		$prejoin[] = "\"$tag\":$str";
	}
	$return_arr["tags_b"]="{".join(",",$prejoin)."}";
